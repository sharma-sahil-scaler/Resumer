...
export function ResumeBuilderFormComponent() {
  ...
  const handleResponsibilityChange = (expIndex: number, respIndex: number, value: string) => {
    setResume(prev => ({
      ...
      work_experience: prev.work_experience.map((exp, i) => 
        i === expIndex 
          ? { ...exp, responsibilities: exp.responsibilities.map((resp, j) => j === respIndex ? value : resp) }
          : exp
      )
    }))
  }
  ...
  const addEducation = () => {
    setResume(prev => ({
      ...prev,
      education: [...prev.education, { institution: '', degree: '', field_of_study: '', start_date: '', end_date: '' }]
    }))
  }
  ...
  const addWorkExperience = () => {
    setResume(prev => ({
      ...prev,
      work_experience: [...prev.work_experience, { company: '', position: '', start_date: '', end_date: '', responsibilities: [''] }]
    }))
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-8 max-w-2xl mx-auto p-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          <Input id="name" name="name" value={resume.name} onChange={handleChange} required />
        </div>
        <div>
          <Label htmlFor="email">Email</Label>
          <Input id="email" name="email" type="email" value={resume.email} onChange={handleChange} required />
        </div>
        <div>
          <Label htmlFor="phone">Phone Number</Label>
          <Input id="phone" name="phone" type="tel" value={resume.phone} onChange={handleChange} required />
        </div>
        <div>
          <Label htmlFor="summary">Summary</Label>
          <Textarea id="summary" name="summary" value={resume.summary} onChange={handleChange} required />
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {resume.work_experience.map((exp, expIndex) => (
          <div key={expIndex} className="space-y-2 p-4 border rounded-md relative">
            <Button
              type="button"
              variant="ghost"
              size="icon"
            ...
            <Input
              ...
              value={exp.position}
              onChange={(e) => handleWorkExperienceChange(expIndex, 'position', e.target.value)}
              required
            />
            <div className="grid grid-cols-2 gap-2">
              <Input
            ...
            <div className="space-y-2">
              ...
              <Button type="button" variant="outline" onClick={() => addResponsibility(expIndex)}>
